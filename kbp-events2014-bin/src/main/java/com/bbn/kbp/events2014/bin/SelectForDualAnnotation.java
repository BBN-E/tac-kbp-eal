...
public final class SelectForDualAnnotation {
    ...
    private static void trueMain(String[] argv) throws IOException {
        ...
        final Set<Symbol> docIDsToIgnore = ImmutableSet.copyOf(FileUtils.loadSymbolList(excludeListFile));
        final AnnotationStore annotationStore = AssessmentSpecFormats.openAnnotationStore(annotationStoreDirectory,
                AssessmentSpecFormats.Format.KBP2014);
        ...
        final ImmutableMap<Symbol, Integer> docIDsToNumResponses = buildDocIDsToNumResponses(annotationStore);

        final Set<Symbol> docIDs = Sets.difference(annotatorsTable.rowKeySet(), docIDsToIgnore).immutableCopy();
        ...
        for (final Map.Entry<String, Set<Symbol>> config :
            ...
            final ImmutableMultimap<Symbol, Symbol> documentsToEventTypes = eventTypesToDocuments.inverse();
            final ImmutableMultimap<Symbol, Symbol> documentsToAnnotators = annotatorsToDocuments.inverse();

            for (final Symbol eventType : eventTypesToDocuments.keySet()) {
                if (!selectedEventTypes.contains(eventType)) {
                    final Symbol selectedDocID = selectClosestTo(eventTypesToDocuments.get(eventType), docIDsToNumResponses, medianSize);
                    ...
                    log.info("For event type {}, selected {} which covers event types {} and annotators {}", eventType, selectedDocID,
            ...
            for (final Symbol annotator : annotatorsToDocuments.keySet()) {
                if (!selectedAnnotators.contains(annotator)) {
                    final Symbol selectedDocID = selectClosestTo(annotatorsToDocuments.get(annotator), docIDsToNumResponses, medianSize);
                    selectedDocIDs.add(selectedDocID);
                    selectedEventTypes.addAll(documentsToEventTypes.get(selectedDocID));
                    selectedAnnotators.addAll(documentsToAnnotators.get(selectedDocID));
                    log.info("For annotator {}, selected {} which covers event types {} and annotators {}", annotator, selectedDocID,
                            documentsToEventTypes.get(selectedDocID), documentsToAnnotators.get(selectedDocID));
                } else {
            ...
            while (selectedDocIDs.size() < 25) {
                final Symbol fillUpDocID = selectClosestTo(unselectedDocIDs, docIDsToNumResponses, medianSize);
    ...
    private static ImmutableMultimap<Symbol, Symbol> makeAnnotatorsToDocumentsMap(Table<Symbol, Symbol, Symbol> annotatorsTable, Set<Symbol> targetDocIDs) {
    ...
    private static ImmutableMultimap<Symbol, Symbol> makeEventTypesToDocumentsMap(AnnotationStore annotationStore, Set<Symbol> targetDocIDs) throws IOException {
        final ImmutableMultimap.Builder<Symbol, Symbol> ret = ImmutableMultimap.builder();
    ...
    private static ImmutableMap<Symbol, Integer> buildDocIDsToNumResponses(AnnotationStore annotationStore) throws IOException {
    ...
    private static double determineMedianSize(AnnotationStore annotationStore, Set<Symbol> targetDocIDs) throws IOException {
