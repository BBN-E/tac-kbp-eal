...
/**
 * Does the second-phase of two-phase assessment. In two-phase assessment (as was used for the 2014 evaluation),
 * the assessors first do coreference and then this coreference information is used to do more aggressive
 * downselection during pruning (this is possible because in the first phase each system was allowed only
 * to keep the best scoring justification for each type-role-filler-realis.  In the second phase, we keep
 * only the best justification for each type-role-*coreffed filler*-realis.
 *
 * This is intended to be run multiple times as the first phase annotation store is filled out. Once a
 * document is written to the second-phase annotation store, it will never be written again, even if the
 * first phase annotation has changed.  Only documents with complete first-phase coreference will be
 * ported over to the second-phase annotation store.
 *
 */
public final class SecondPhaseAssessmentPooler {
    ...
    private static void trueMain(String[] argv) throws IOException {
        ...
        final File firstPhaseAnnotationStorePath = params.getCreatableDirectory("firstPhaseAnnotationStore");
        final File outputAnnotationStorePath = params.getCreatableDirectory("secondPhaseAnnotationStore");
        ...
        final AnnotationStore firstPhaseAnnotationStore = AssessmentSpecFormats.openAnnotationStore(firstPhaseAnnotationStorePath, fileFormat);
        ...
        final Set<Symbol> docsToImport = Sets.difference(docIDsWithCompleteCoref, outputAnnotationStore.docIDs());
        for (final Symbol docId : docsToImport) {
            ...
            log.info("\nFor document {}\n{}\n{} responses total", docId, sb.toString(), combinedOutput.allResponses().size());
        }
    ...
    private static AnswerKey responsesNeededToScore(Iterable<SystemOutput> systemOutputs, AnswerKey answerKey) {
        ...
        final SystemOutput pooledResponsesUsingCoref = SystemOutput.unionKeepingMaximumScore(locallyBestUsingCoref);
    ...
    private static AnswerKey toAnswerKeyWithCoref(SystemOutput systemOutput, CorefAnnotation corefAnnotation) {
        final Set<KBPString> allCASesInResponses = FluentIterable.from(systemOutput.responses())
                .transform(Response.CASFunction())
                .toSet();

        final CorefAnnotation corefOnOnlyRelevantCASes = corefAnnotation.copyRemovingStringsNotIn(allCASesInResponses);
        return AnswerKey.from(systemOutput.docId(), ImmutableSet.<AssessedResponse>of(), systemOutput.responses(),
                corefOnOnlyRelevantCASes);
    }

    private static Set<Symbol> docIDsWithCompleteCoref(AnnotationStore annotationStore) throws IOException {
