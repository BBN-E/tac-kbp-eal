...
import com.bbn.kbp.events2014.filters.KeepBestJustificationOnly;
import com.bbn.kbp.events2014.filters.ProbableInferenceCases;
    ...
    private static void trueMain(String[] argv) throws IOException {
        ...
        for (final Symbol docID : sourceStore.docIDs()) {
            ...
            if (keepInferenceCases) {
                toOutput = SystemOutput.unionKeepingMaximumScore(ImmutableList.of(filtered, probableInferenceCases));
                if (toOutput.size() > filtered.size()) {
                    log.info("For document {}, filtered out {} responses as duplicate justifications, but restored {} as probably inference cases",
