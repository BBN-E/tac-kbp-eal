...
public final class PercentileComputer {
    ...
    /**
     * Creates a {@code PercentileComputer} which uses the algorithm given in NIST's
     *  Engineering Statistics Handbook. The relevant section is copied below:
     *
     * <blockquote>
     *  Percentiles can be estimated from {@code N} measurements as follows:
     *  for the pth percentile, set p(N+1) equal to k + d for k an integer,
     *  and d, a fraction greater than or equal to 0 and less than 1.
     *  <ul>
     * <li>For 0 < k < N,  Y(p) = Y[k] + d(Y[k+1] - Y[k])</li>
     * <li>For k = 0,  Y(p) = Y[1]</li>
     * <li>For k = N,  Y(p) = Y[N]</li>
     * </ul>
     *</blockquote>
     *
    ...
    /**
     * Creates a {@code PercentileComputer} which uses the "Excel" alternative algorithm
     * given in NIST's Engineering Statistics Handbook.  It is the same as the base NIST
     * algorithm, except it assigns (k,d) to the integral and fractional parts of p(N-1)+1
     * instead of p(N+1).
     *
     * @see http://www.itl.nist.gov/div898/handbook/prc/section2/prc252.htm
     */
  ...
  public Percentiles calculatePercentilesCopyingData(double[] data) {
    ...
    public class Percentiles {
        ...
        /**
         * Calculates the p-th percentile of the observed data.  The algorithm used varies
