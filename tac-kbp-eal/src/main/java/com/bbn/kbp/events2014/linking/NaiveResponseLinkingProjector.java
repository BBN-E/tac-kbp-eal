...
/**
 * Projects a source response linking onto a target answer key possibly different than the one it
 * originated on  using a very simple algorithm: <ul> <li>Each cluster in the source response
 * linking is projected to a new cluster by mapping each response in the source cluster to its
 * equivalence class under the target answer key and then taking all the responses in that
 * equivalence class. Note that it may not be possible to map from a source response to a target
 * equivalence class (e.g. the target deleted some responses), in which case the source response is
 * just ignored. </li> <li>The incomplete response set in the projection is all assessed responses
 * from the target answer key which did not end up in any cluster.</li> </ul>
 *
 * Note that this algorithm can behave in dangerous and unexpected ways. For example, suppose the
 * target answer key differs from the original only by the annotator splitting a coreference
    ...
    for (final Symbol docID : sourceLinkingStore.docIDs()) {
      ...
      if (sourceResponseLinkingOpt.isPresent()) {
        ...
        final AnswerKey targetAnswerKey = targetAnnotationStore.read(docID)
            .filter(LinkingUtils.linkableResponseFilter2015ForGold());
