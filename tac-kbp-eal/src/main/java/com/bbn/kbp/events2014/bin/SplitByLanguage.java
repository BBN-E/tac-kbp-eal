...
public final class SplitByLanguage {
  ...
  private static void trueMain(String[] argv) throws IOException {
    ...
    for (final File file : inputDir.listFiles()) {
      if (file.isDirectory()) {
        try (final SystemOutputStore2016 input = SystemOutputStore2016.open(file)) {
          ...
          for (final Map.Entry<String, Pattern> e : LANGUAGE_TO_PATTERN.entrySet()) {
            ...
            final File outputDir = new File(new File(baseOutputDir, language), file.getName());
            try (final SystemOutputStore2016 output = SystemOutputStore2016.openOrCreate(outputDir)) {
              filter(input, output, filterPattern);
            }
          }
        }
      }
  ...
  private static void filter(final SystemOutputStore2016 input, final SystemOutputStore2016 output,
